...
Total number is the sum, or 84 patients.

- What is the probability a randomly selected patient received the placebo?

The probability of a patient receiving the placebo is 43/84 (~52%).

- What is the probability a randomly selected patient received the placebo and had a marked improvement?

7/84.
  
- What is the probability a randomly selected patient received the placebo and the treatment?
  
0/84. (Not possible; disjoint probability)
...
are equally likely to be selected. What is the probability the marble is blue? 3/8
How about white? 1/8
...
Close, but not exact. Ex. white is 0.1255 vs 0.1250 for 1/8, red is 0.4910 vs 0.5000. Law of large numbers -> increasing sample size approaches "true" probability.
...
Guess: less likely to get same color twice.
...
Use mutate to create a T/F variable.
...
roll_results <-
  roll_results %>%
  mutate(event_1 = roll_1^2 == roll_2, 
        event_2 = abs(roll_1 - roll_2) == 4)
...
```{r summarize_rolls}
roll_results %>%
  summarise(event_1_prob = mean(event_1), event_2_prob = mean(event_2))
```
